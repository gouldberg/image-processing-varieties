...
# --------------------------------------------------------------------------------------------------
# elastic transform with affile transform
# --------------------------------------------------------------------------------------------------
...
def elastic_transform(img, alpha, sigma, alpha_affine,
    ...
    return cv2.remap(img, map1=np.float32(x + dx), map2=np.float32(y + dy),
                     interpolation=interpolation, borderMode=border_mode, borderValue=value)*255
...
def elastic_transform_simple2(image, severity=1,
                              interpolation=cv2.INTER_LINEAR, border_mode=cv2.BORDER_REFLECT_101, value=None):
    ...
    img_remap = cv2.remap(image, map1=np.float32(x + dx), map2=np.float32(y + dy),
                        interpolation=interpolation, borderMode=border_mode, borderValue=value)
    distorted_img = (np.clip(img_remap, 0, 1)*255).astype('uint8')
...
for i in range(len(img_file_list)):
    ...
    img_trans_rgb1 = elastic_transform(img_rgb, alpha=alpha, sigma=sigma, alpha_affine=alpha_affine)
    # img_trans_rgb1 = elastic_transform(img_rgb, alpha=alpha1, sigma=sigma1, alpha_affine=alpha_affine, approximate=True)
    # ----------
    img_to_show = np.hstack([img_rgb, img_trans_rgb1])
    # ----------
    for i in range(5):
        img_trans_rgb2 = elastic_transform_simple2(img_rgb, severity=i+1).astype('uint8')
        img_to_show = np.hstack([img_to_show, img_trans_rgb2])
    # ----------
