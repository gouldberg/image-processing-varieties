...
img_file_list = sorted(glob.glob(os.path.join(image_dir, 'dehaze/*')))
...
for i in range(len(img_file_list)):
    ...
    J = Recover(I,t,A,0.1)
    # ----------
    dark_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    t_rgb = cv2.cvtColor((t*255).astype('uint8'), cv2.COLOR_BGR2RGB)
    J_rgb = cv2.cvtColor((J*255).astype('uint8'), cv2.COLOR_BGR2RGB)
    # ----------
    HazeCorrectedImg, haze_map = remove_haze(img, showHazeTransmissionMap=False)
    HazeCorrectedImg_rgb = cv2.cvtColor(HazeCorrectedImg, cv2.COLOR_BGR2RGB)
    haze_map_rgb = cv2.cvtColor((haze_map*255).astype('uint8'), cv2.COLOR_BGR2RGB)
    # ----------
    img_to_show = np.hstack([img_rgb, J_rgb, HazeCorrectedImg_rgb, haze_map_rgb])
